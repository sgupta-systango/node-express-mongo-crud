...
module.exports.edit = async(req, res, next) => {
    try {
        const { id } = req.query;
        const errid = req.flash('id')
        const result = await allProducts.findOne({ $or: [{ _id: id }, { _id: errid }] });
        res.render('adminEditProduct', { data: result, uid: req.session.user._doc.email, isAdmin: req.session.user.isAdmin, msg1: req.flash('msg1') });
    } catch (err) {
        res.json({ error: err.toString() })
    }
}
